...
# test['test_err'] = test_err
# test.to_csv(cwd+"/DATA/test_err.csv", index = False, index_label = False)
# print("saved test error")
...
#     trainY_pred = alg.predict(trainX)
#
#     #Print model report:
#     print("Explained Variance : %.4f " % (explained_variance_score(trainY, trainY_pred)))
#     print("RMSE : %.4f" % (mean_squared_error(trainY, trainY_pred) ** .5))
#
#     train_err = np.absolute(trainY - trainY_pred)/trainY
#     train_err_mean = np.mean(train_err)
#
#     print ("Absolute Error rate: %.4f" % train_err_mean)
#
#     train_err_1 = np.sum((train_err < .1)*1)/len(train_err)
#     train_err_2 = np.sum((train_err < .2)*1)/len(train_err)
#     print("%.2f within 0.1, %.2f within 0.2" % (train_err_1, train_err_2))
#
#
#
#     # a = trainY.head(n=10).values
#     # b = predY[0:10]
#     # print (a)
#     # print (b)
#
#     feat_imp = pd.Series(alg.feature_importances_, list(trainX)).sort_values(ascending=False)
#     feat_imp.plot(kind='bar', title='Feature Importances')
#     plt.ylabel('Feature Importance Score')
#     plt.show()
...
#        reg_alpha=0, reg_lambda=1, scale_pos_weight=1, seed=None,
#        silent=True, subsample=0.95498622807161138)
# RMSE : 177828.6636 (train) 427731.1156 (test)
# MeanAE : 49964.6111 (train) 82460.0968 (test)
# Error rate within 0.01: 0.06 (train), 0.04 (test)
# Error rate within 0.05: 0.28 (train), 0.21 (test)
# Error rate within 0.10: 0.50 (train), 0.40 (test)
# Error rate within 0.20: 0.77 (train), 0.66 (test)
# Error rate within 0.30: 0.88 (train), 0.80 (test)
# Error rate within 0.40: 0.93 (train), 0.87 (test)
# Error rate within 0.50: 0.96 (train), 0.92 (test)
# Error rate within 0.60: 0.97 (train), 0.94 (test)
# Error rate within 0.70: 0.98 (train), 0.95 (test)
# Error rate within 0.80: 0.98 (train), 0.96 (test)
# Error rate within 0.90: 0.99 (train), 0.97 (test)
# Error rate within 1.00: 0.99 (train), 0.98 (test)
